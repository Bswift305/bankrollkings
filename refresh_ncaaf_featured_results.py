...
def main() -> int:
    ...
    gamelog_map = {"NCAAF": load_ncaaf_gamelogs()}
    snapshot = write_featured_results_snapshot_for_sport("NCAAF", archive_df=archive_df, gamelog_map=gamelog_map)
    sport_archive = archive_df.copy()
    if archive_df is not None and not archive_df.empty and "Sport" in archive_df.columns:
        sport_archive = archive_df[archive_df["Sport"].astype(str).str.upper() == "NCAAF"].copy()
    ...
    report = {
        ...
        "notes": (
            f"Wrote {len(snapshot)} NCAAF featured result rows. "
            f"Resolved {summary.get('totals', {}).get('resolved', 0)} of "
            f"{summary.get('totals', {}).get('candidates', 0)} archived featured plays."
        ),
    }
    append_qc_run_log("ncaaf_featured_results", report)
    print("=" * 60)
    print("NCAAF FEATURED RESULTS SNAPSHOT")
    print("=" * 60)
