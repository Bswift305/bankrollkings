...
- **RAM is the real ceiling and it is tight.** **`earlyoom` is active and SIGTERMs
  any process when available memory drops below 12%.** That is not a bug — it is the
  same reaper from the Jul 1-4 outage, still armed. A heavy batch job run alongside
...
- **The daily archiver was also trimmed** (`archive_daily_candidates.py`, 2026-07-25):
  ...
  1.67 → 1.28 GB. That plus the one-worker change is what stopped the kills. For other
  heavy jobs, splitting into per-section subprocesses (see prelaunch scorecard in §6) is
  the other lever — not touching earlyoom.
...
- **Quick Tools** (`/tools/*`, 2026-07-25): cross-league utilities that answer a NARROW,
  ...
  `qc_quick_tools.py`** (15-check smoke test, logs to the QC run log). Reachable ONLY from a
  **"Quick Tools" section in the left sidebar** — deliberately NOT in the per-sport
  workflow toolbar (their whole purpose is to keep that toolbar uncluttered; a first pass
  wrongly added them to the toolbar and it was pulled back out). SVG badge icons in
  `static/logos/nav/tools/*.svg` built to match the sport/settings set (dark circle, cyan
  ring, bold cyan glyph). Added to the `bk_base.html` sidebar block AND the
  `dashboard_overview.html` override so it shows on the member dashboard too. Toolbar cluster after Command Center: **Slate Pulse · Market Movers · Best
  ...
  **Model-vs-Market (#8) is deferred** — model probabilities aren't captured consistently
  ...
  - **Injury Report** `/tools/injury-report` (`injury_report_tool`, all_access; the nav
    ...
    NBA/WNBA/MLB/NFL into one client-side-filterable table (league / status / team /
    search); `_injury_severity_bucket()` normalizes each league's free-text status into
    Out/Doubtful/Questionable/Other. **"Impact" (with/without) is live for NBA + NFL:**
    an injured player who drives a teammate split shows an expandable row of "when X
    sits, teammate Y produces Z". NBA from `Active_Boost_Plays.csv`; NFL from
    `NFL_Active_Boost_Plays.csv` (generated by `fetch_injuries.run_nfl_boost_update()`
    — position-gated, current-team-matched, from gamelogs). **MLB/WNBA impact NOT built**
    (needs per-sport gamelog split engines). See [[project_quick_tools]].
- **Review Center** `/candidate-review` now shows **ROI beside hit rate** (2026-07-25):
  ...
  slightly-negative at pick price despite 60-76% hit rates (favorite skew). Sort is still
  by hit rate; ROI shown, not yet the ranking key.
...
- **Injury Report "impact" (with/without) — NBA + NFL live, MLB/WNBA not built
  (2026-07-25).** The `/tools/injury-report` table and its NBA+NFL teammate-split
  "impact" rows are done (see §4). Extending the with/without insight to **MLB and
