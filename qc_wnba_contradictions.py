...
from services.wnba_contradiction_qc import audit_props, prop_key
...
def build_runtime() -> dict:
    ...
    scored_props = build_wnba_prop_board(
        props_df,
        odds_df,
        schedule_df,
...
def run_qc(persist=True) -> dict:
    ...
    if persist:
        update_warning_history("wnba_contradictions", report["warnings"], report["failures"], report["checked_at"])
        append_qc_run_log("wnba_contradictions", report)
    return report


def main() -> int:
    parser = argparse.ArgumentParser(description="Bankroll Kings WNBA contradiction QC")
