...
ROUTES = (
    ("/sports/mlb", ("Daily Slate Command", "Baseball Expansion")),
    ("/sports/mlb/market-edge", ("MLB Market Edge", "Current Baseball State")),
    ("/sports/mlb/floor", ("MLB Floor Plays", "Current Baseball State")),
    ("/sports/mlb/trends", ("MLB Trends", "Current Baseball State")),
)
...
def run_qc() -> dict:
    ...
    if missing_markets:
        missing_labels = ", ".join(row.get("stat", "") for row in missing_markets)
    ...
    if context_df.empty:
        ...
        if "Temperature" in context_df.columns and context_df["Temperature"].fillna("").astype(str).str.strip().eq("").all():
            warnings.append("MLB game context has no weather feed yet.")
        if "Umpire" in context_df.columns and context_df["Umpire"].fillna("").astype(str).str.strip().eq("").all():
            warnings.append("MLB game context has no umpire assignment feed yet.")
