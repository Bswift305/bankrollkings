...
def main() -> int:
    ...
    gamelog_map = {"MLB": load_mlb_gamelogs()}
    snapshot = write_featured_results_snapshot_for_sport("MLB", archive_df=archive_df, gamelog_map=gamelog_map)
    sport_archive = archive_df.copy()
    if archive_df is not None and not archive_df.empty and "Sport" in archive_df.columns:
        sport_archive = archive_df[archive_df["Sport"].astype(str).str.upper() == "MLB"].copy()
    ...
    report = {
        ...
        "notes": (
            f"Wrote {len(snapshot)} MLB featured result rows. "
            f"Resolved {summary.get('totals', {}).get('resolved', 0)} of "
            f"{summary.get('totals', {}).get('candidates', 0)} archived featured plays."
        ),
    }
    append_qc_run_log("mlb_featured_results", report)
    print("=" * 60)
    print("MLB FEATURED RESULTS SNAPSHOT")
    print("=" * 60)
